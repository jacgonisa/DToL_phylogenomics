...
uw_raw <- readLines(file.path(SP_DIR, "groupsim_sat_trans/groupsim_sat_trans_gap08.txt"))
...
wt <- read.delim(file.path(SP_DIR,
                            "groupsim_sat_trans_weighted/groupsim_st_clade_gap08.tsv"),
...
helix_raw <- readLines(file.path(SP_DIR,
                                  "groupsim_sat_trans/helix_positions_gap08.txt"))
...
p_zscore <- ggplot(df_z, aes(x=pos, y=0.5, fill=z_score)) +
  ...
  scale_x_continuous(limits=c(x_min,x_max), expand=c(0,0),
                     name="Alignment position (trimmed, gap <= 80%)") +
  scale_y_continuous(limits=c(0,1), expand=c(0,0)) +
  theme_void(base_size=9) +
...
ggsave(file.path(FIG_DIR,"groupsim_sat_vs_trans_gap08_pub.pdf"),
       p_out, width=13, height=6.5, bg="white")
ggsave(file.path(FIG_DIR,"groupsim_sat_vs_trans_gap08_pub.png"),
       p_out, width=13, height=6.5, dpi=300, bg="white")
cat("Saved: groupsim_sat_vs_trans_gap08_pub.{pdf,png}\n")
