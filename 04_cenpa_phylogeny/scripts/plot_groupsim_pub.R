...
z_abs <- max(abs(df$z_score), na.rm=TRUE)

# ── Panel 1: helix track ──────────────────────────────────────────────────────
helix$ymin <- ifelse(helix$histone == "CENPA", 0.55, 0.05)
helix$ymax <- ifelse(helix$histone == "CENPA", 0.95, 0.45)
helix$col  <- ifelse(helix$histone == "CENPA", C_CENPA, C_H3)

p_helix <- ggplot(helix) +
  geom_rect(aes(xmin=start-0.5, xmax=end+0.5, ymin=ymin, ymax=ymax,
                fill=histone), alpha=0.85, colour=NA) +
  scale_fill_manual(values=c(CENPA=C_CENPA, H3=C_H3),
                    labels=c("CENP-A helices","H3 helices"), name=NULL) +
...
p_main <- ggplot() +
  ...
  geom_hline(yintercept=z2_thresh, colour=C_SIG,
...
p_zscore <- ggplot(df_z, aes(x=pos, y=0.5, fill=z_score)) +
  ...
  scale_fill_gradient2(low="#2166ac", mid="white", high="#c62828",
                       midpoint=0, limits=c(-z_abs,z_abs), name="z-score",
...
p_out <- p_helix / p_main / p_zscore +
  plot_layout(heights=c(0.7, 6, 0.5)) +
  plot_annotation(
...
ggsave(file.path(FIG_DIR,"groupsim_cenpa_vs_h3_pub.pdf"),
       p_out, width=13, height=6.5, bg="white")
ggsave(file.path(FIG_DIR,"groupsim_cenpa_vs_h3_pub.png"),
       p_out, width=13, height=6.5, dpi=300, bg="white")
cat("Saved: groupsim_cenpa_vs_h3_pub.{pdf,png}\n")
