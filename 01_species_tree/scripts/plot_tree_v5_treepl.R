...
#
# Output -> 01_species_tree/figures/centromere_annotation_tree_FASTSPECIES_325sp_v1_treePL.{pdf,png}
...
tree_path  <- file.path(TR_DIR, "full_325sp_treepl_over_fa.nwk")
out_pdf    <- file.path(FIG_DIR, "centromere_annotation_tree_FASTSPECIES_325sp_v1_treePL.pdf")
out_png    <- file.path(FIG_DIR, "centromere_annotation_tree_FASTSPECIES_325sp_v1_treePL.png")
...
# ── Save outputs ──────────────────────────────────────────────────────────────
pdf(out_pdf, width = 14, height = 18)
print(p)
dev.off()

png(out_png, width = 1800, height = 2300, res = 250)
print(p)
dev.off()
